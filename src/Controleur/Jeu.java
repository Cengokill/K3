...
public class Jeu {
	...
	public Jeu(JFrame fenetrePrincipale, InitPartie partieInit, LoadTexture t) {
		...
		plateau = new Plateau();
		setParametresPartie(partieInit.modeDeJeu, partieInit.difficulteIA1, partieInit.difficulteIA2, 2000,
				partieInit.nomJoueur1, partieInit.nomJoueur2);
		// lancer une partie
		/*
		 ...
		 * System.exit(0);
		 * setParametresPartie(partieInit.modeDeJeu,partieInit.difficulteIA1,partieInit.
		 * difficulteIA2,2000,partieInit.nomJoueur1,partieInit.nomJoueur2);
		 * }
		 * }else {
		 * setParametresPartie(partieInit.modeDeJeu,partieInit.difficulteIA1,partieInit.
		 * difficulteIA2,2000,partieInit.nomJoueur1,partieInit.nomJoueur2);
	...
	public void lancerPartie() {
		// this.simpleSoundPlayerMusic.stopSound();//stopper la mussique d'accueil du
		// jeu
	...
	public void initPhase1() {
		...
		while (this.partieEnCours.joueur1().getTaillePiecesPiochees() < this.TAILLE_CAMP_JOUEUR
				|| this.partieEnCours.joueur2().getTaillePiecesPiochees() < this.TAILLE_CAMP_JOUEUR) {
			piocher();
		}
	}
				...
				if (!arr.isEmpty()) {
					for (PiecePyramide p : arr) {
						...
						if (acteurCourant.getCamp().empilerPhase1(p)) {// pas de verif pieces porteuse
							acteurCourant.getPiecesPiochees().remove(p.getPiece());
							this.panel.repaint();
	...
	public void jouer() {
		...
		while (coupDemande == null) {
			coupDemande = jCourant.jouer(cJ, this.partieEnCours);// le joueur courant a choisi un coup a jouer
		}
		...
		if (coupDemande.getPosBase() != null) {// si le joueur ne choisit pas de jouer une piece BLANCHE
			this.partieEnCours.getBaseMontagne()
					.empiler(new PiecePyramide(coupDemande.getPiece(), coupDemande.getPosBase()));
			if ((this.typeActeurs == 2 && this.vitesseIA > 400) || this.typeActeurs != 2) {
			...
			timer(temps);
			panel.repaint();
			if (this.partieEnCours.getBaseMontagne().estPorteursMemeCouleur(coupDemande.getPosBase())) {// si vol
																										// possible
				if ((this.typeActeurs == 2 && this.vitesseIA > 400) || this.typeActeurs != 2) {
	...
	public void lireOptions() {// au tout premier lancement du jeu, le fichier Options.txt existe deja
		...
		try {
			...
			if (tab.size() != this.NB_LIGNES_OPTIONS) {
				...
				testPhotoProfil = testFichierExistant(chemin + photoProfil + ".jpg")
						|| testFichierExistant(chemin + photoProfil + ".png");
	...
	public boolean chargerPartie(String nomFichier) {
		...
		try {
			...
			if (tab.size() != this.NB_LIGNES_SAUVEGARDE) {
				System.err.println(
						"Erreur : le fichier de sauvegarde ne contient pas le bon nombre de lignes." + tab.size());
				return false;
			}
